    ...
    let track_downloads = if !track_formats_sorted.is_empty() {
        ...
        let track_download_columns = track_formats_sorted
            ...
            .map(|download_format| {
                ...
                let track_file = format!("{format_dir}/{track_hash}/{track_filename_urlencoded}");
                let url = if let Some(cdn_url) = &build.cdn_url {
                    format!("{}{}/{}/{}", cdn_url, release.permalink.slug, track_number, track_file)
                } else {
                    format!("{track_prefix}{track_file}")
                };
                download_entry(
                    url,
